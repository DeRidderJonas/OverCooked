...
{
	float3 halfV = normalize(gLightDirection + viewDirection);
	float specularStrength = max(dot(-halfV, normal), 0.0);
	
	float gloss = gShininess;
	if(gUseTextureSpecularIntensity) gloss *=  
		gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;
	
	float specularReflection = pow(specularStrength, gloss * 4.0);
	
	return gColorSpecular * specularReflection;
}

float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	float3 reflectV = normalize(reflect(gLightDirection, normal));
	float specularStrength = saturate(dot(reflectV, -viewDirection));
	
	float gloss = gShininess;
	if(gUseTextureSpecularIntensity) gloss *=  
		gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;
	
	float specularReflection = pow(specularStrength, gloss);
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	else if(gUseSpecularBlinn) specColor = CalculateSpecularBlinn(viewDirection, normal, texCoord);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if(gUseTextureNormal)
	{
		...
		sampledNormal = 2 * sampledNormal - float3(1,1,1);
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	return diffColor * diffuseStrength;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	{
		if(gUseEnvironmentMapping) return environmentColor;
		else return float3(0,0,0);
	}
	
	float fresnel = pow((1 - saturate(abs(dot(normal, viewDirection)))), gFresnelPower) 
						* gFresnelMultiplier;
	float mask = pow(1 - saturate(dot(float3(0,-1,0), normal)), gFresnelHardness);
	
	float3 color = float3(0,0,0);
	if(gUseEnvironmentMapping) color = fresnel * mask * environmentColor;
	else color = fresnel * mask * gColorFresnel;
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 refractV = refract(viewDirection, normal, gRefractionIndex);
	
	float3 reflectColor = gCubeEnvironment.Sample(gTextureSampler, reflectV);
	float3 refractColor = gCubeEnvironment.Sample(gTextureSampler, refractV);
	
	return reflectColor * gReflectionStrength + refractColor * gRefractionStrength;
}
