...
void RigidBodyComponent::SetLinearVelocity(const physx::PxVec3& velocity, bool autoAwake) const
{
	if (m_pActor != nullptr && !m_isStatic && !m_isKinematic)
		reinterpret_cast<physx::PxRigidDynamic*>(m_pActor)->setLinearVelocity(velocity, autoAwake);
	else
		Logger::LogError(L"[RigidBodyComponent] Cannot set linear velocity to a static or kinematic actor");
}

physx::PxVec3 RigidBodyComponent::GetLinearVelocity() const
{
	if (m_pActor != nullptr && !m_isStatic && !m_isKinematic)
		return reinterpret_cast<physx::PxRigidDynamic*>(m_pActor)->getLinearVelocity();
	else
		Logger::LogError(L"[RigidBodyComponent] Cannot get linear velocity from a static or kinematic actor");
#if _DEBUG
	return physx::PxVec3{};
#endif
}
