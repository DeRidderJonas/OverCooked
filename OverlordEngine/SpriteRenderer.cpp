...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if(!m_pVertexBuffer)
	{
		...
		bufferDesc.ByteWidth = sizeof(SpriteVertex) * (uint32_t)m_BufferSize;
		bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		bufferDesc.MiscFlags = 0;

		result = gameContext.pDevice->CreateBuffer(&bufferDesc, 0, &m_pVertexBuffer);
		if(FAILED(result))
		{
			Logger::LogError(L"SpriteRenderer::UpdateBuffer > CreateBuffer failed");
			return;
		}
	}
	...
	memcpy(mappedResource.pData, m_Sprites.data(), m_Sprites.size() * sizeof(SpriteVertex));
